"""
graph_transformer_policy_dgl.py
This module implements a graph-based policy network for topology manipulation using DGL and PyTorch. 
It is designed for environments where both node-level and graph-level features are important for decision making, 
such as reinforcement learning tasks involving dynamic graphs.
Key Components:
---------------
- GraphTransformerEncoder:
    A neural network encoder that processes both node features and graph-level features using stacked GATv2Conv layers.
    It produces enhanced node embeddings and a fused graph embedding. The encoder expects node_embeddings and 
    graph_embeddings as input, which are typically generated by the TopologyState class in state.py.
- GraphPolicyNetwork:
    A policy network built on top of the GraphTransformerEncoder. For each node, it outputs action logits 
    (spawn/delete) and continuous parameters (gamma, alpha, noise, theta) for topology actions. It supports 
    both deterministic and stochastic action selection.
- TopologyPolicyAgent:
    An agent class that integrates the topology, state extractor, and policy network. It retrieves the full state 
    embedding from the embedding (using node_embeddings and graph_embeddings from state.py), 
    runs the policy network, and executes topology actions (spawn/delete) with the predicted parameters.
Usage:
------
The file includes an integration test in the `__main__` block, demonstrating how to set up the substrate, topology, 
embedding, and policy network. It shows how to retrieve the full embedding state, run the policy, and execute 
actions to evolve the graph topology over multiple cycles.
Dependencies:
-------------
- DGL (Deep Graph Library)
- PyTorch
- state.py (for TopologyState, which provides node_embeddings and graph_embeddings)
- topology.py, substrate.py (for environment setup)
Highlights:
-----------
- The GraphTransformerEncoder relies on node_embeddings and graph_embeddings from state.py to provide 
  context-aware representations for both nodes and the overall graph.
- The policy network uses these embeddings to make topology-altering decisions in a flexible and scalable manner.
"""
...
class GraphPolicyNetwork(nn.Module):
    ...
    def forward(self, state_dict: Dict[str, torch.Tensor], deterministic: bool = False, max_gamma: float = 5.0, max_alpha: float = 2.0, max_noise: float = 1.0) -> Dict[str, torch.Tensor]:
        ...
        Args:
            state_dict: Full state from state_extractor.get_state_features()
...
class TopologyPolicyAgent:
    """
    Agent that combines our topology with the graph transformer policy. Also, it uses full state features.
    """
    ...
    def get_policy_actions(self, embedding_dim: int = 64, deterministic: bool = False) -> Tuple[Dict[int, str], Dict[int, Tuple[float, float, float, float]]]:
        ...
        """
        # Get FULL state features (not just DGL graph)
